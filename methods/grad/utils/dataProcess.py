...
def mergeGraphDataList(args, graph_pyg, syn_relation_dict):
    ...
    cum_num_nodes = 0  # Cumulative number of nodes, used to update edge_index
    ...
    if args.dataset=='blogcatalog' and args.GuiDDPM_sample_with_guidance: # blogcatalog needs dimensionality reduction
...
def GDCAugment(graph_pyg_type, avg_degree):
    ...
    gdc_transformed_data = gdc(graph_pyg_type)  # apply GDC transform
...
def data4WFusionTrain(graph_pyg, graph_syn, graph_gdc_list):
    ...
    # Explicitly specify number of nodes when creating the heterograph
